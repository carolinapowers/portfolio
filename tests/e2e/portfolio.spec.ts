...
test.describe('Portfolio E2E Tests', () => {
  test('should load the homepage', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Interactive Portfolio' })
    ).toBeVisible();
    await expect(page.getByText('Carolina Powers')).toBeVisible();
  });
  ...
  test('should have working portfolio source link', async ({ page }) => {
    ...
    await expect(sourceLink).toHaveAttribute(
      'href',
      'https://github.com/carolinapowers/portfolio'
    );
  });
  ...
  test('should show recommendations section', async ({ page }) => {
    await page.goto('/');
    await expect(
      page.getByRole('heading', { name: "What It's Like to Work With Me" })
    ).toBeVisible();
    await expect(
      page.getByText(
        "The best way to understand what it's like to work with me is by reading what the people I've collaborated with have to say about the experience."
      )
    ).toBeVisible();
  });
  ...
  test('should show design system section', async ({ page }) => {
    ...
    await expect(storybookBtn).toHaveAttribute(
      'href',
      'https://design-system-iota-five.vercel.app/?path=/docs/components-composite-modal--docs'
    );
    await expect(githubBtn).toBeVisible();
    await expect(githubBtn).toHaveAttribute(
      'href',
      'https://github.com/carolinapowers/design-system'
    );
  });
